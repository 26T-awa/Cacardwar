function cacardwar:main/recover_all_slot_tag
tag @s remove cacard.shield_fire
tag @s remove cacard.shield_sword
tag @s remove cacard.hungry
tag @s remove cacard.regenerating
tag @s remove cacard.godn_head_rege1
tag @s remove cacard.godn_head_rege2
tag @s remove cacard.godn_head_rege3
tag @s remove cacard.bless_of_crow

tag @s remove cacard.attack
tag @s remove cacard.average
tag @s remove cacard.resist
tag @s remove cacard.summon
tag @s remove cacard.blueTeam
tag @s remove cacard.redTeam
tag @s remove cacard.winner
tag @s remove cacard.ready

tag @s remove cacard.newPlayer
tag @s remove cacard.bt
tag @s remove cacard.bt1
tag @s remove cacard.bt2
tag @s remove cacard.bt3
tag @s remove cacard.bt4
tag @s remove cacard.bt100

tag @s remove cacard.ai_clean
tag @s remove cacard.ai_place
tag @s remove cacard.ai_placed_card