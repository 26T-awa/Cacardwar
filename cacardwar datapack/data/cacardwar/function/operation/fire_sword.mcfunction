...
$tellraw @a[tag=cacard.ingame] [{text:"\n §7[§b§l牌§6§l牌§r§a大作战§7]   §$(teamtext)方"},{selector:"@a[tag=cacard.$(team)Team,limit=1]",color:$(teamcolor),underlined:true},{text:"§e合成了§6烈焰剑"},{text:"   §7[",extra:[{score:{name:"#cacard.time",objective:"cacard.time"},hover_event:{action:"show_text",value:"时间戳"}},{text:"§7]"}]}]
...
$execute if entity @a[tag=cacard.$(oppteam)Team,scores={cacard.shieldtype=0..2}] run scoreboard players remove #cacard.$(oppteam)Health cacard.health 5
$execute if entity @a[tag=cacard.$(oppteam)Team,scores={cacard.shieldtype=4}] run scoreboard players remove #cacard.$(oppteam)Health cacard.health 5
execute if score #cacard.time cacard.isongoing matches 5 run scoreboard players remove #cacard.redHealth cacard.health 5
execute if score #cacard.time cacard.isongoing matches 5 if score #cacard.redHealth cacard.health matches 10.. run function cacardwar:ai/calculation/rule9 {arg1:5,arg2:0,arg3:0}
execute if score #cacard.time cacard.isongoing matches 5 if score #cacard.redHealth cacard.health matches 5..9 run function cacardwar:ai/calculation/rule9 {arg1:5,arg2:1,arg3:0}
execute if score #cacard.time cacard.isongoing matches 5 if score #cacard.redHealth cacard.health matches ..4 run function cacardwar:ai/calculation/rule9 {arg1:5,arg2:1,arg3:1}
